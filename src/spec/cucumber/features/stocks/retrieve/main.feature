Feature: Retrieve stocks by ID
  ...
  Scenario: Retrieve non-existing company
    When the client sends a GET request to /stocks/non-existent-company
    And sends the request
    Then the API should respond with a 404 HTTP status code
    And the payload of the response should be a JSON object
    And contains a message property which says "Incorrect company index"
